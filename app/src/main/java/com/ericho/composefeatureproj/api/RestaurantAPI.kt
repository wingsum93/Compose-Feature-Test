...
interface RestaurantAPI {
    ...
    suspend fun getTicket(@Path("member") member: Int): Response<TicketResponse>

    @GET("/queue/{queue_code}")
    suspend fun checkSingleQueue(@Path("queue_code") queue_code: String): Response<SingleTicketResponse>

    @GET("/queue/updateState/member/{member}")
    suspend fun checkQueueOfType(@Path("member") member: Int): Response<TicketResponse>
}